...
public class ApiExceptionHandler {
	...
	{
		ExceptionResponse exceptionResponse = ExceptionResponse.builder().datetime(LocalDateTime.now()).errorMessage(passwordException.getPasswordError()).status(HttpStatus.BAD_REQUEST).build();
		return ResponseEntity.badRequest().body(exceptionResponse);
	}
	@ExceptionHandler(EmailException.class)
	public ResponseEntity<ExceptionResponse> handlerEmailException(EmailException emailException)
	{
		ExceptionResponse exceptionResponse = ExceptionResponse.builder().datetime(LocalDateTime.now()).errorMessage(emailException.getEmailError()).status(HttpStatus.BAD_REQUEST).build();
		return ResponseEntity.badRequest().body(exceptionResponse);
	}
	@ExceptionHandler(UsernameException.class)
	public ResponseEntity<ExceptionResponse> handlerUsernameException(UsernameException usernameException)
	{
		ExceptionResponse exceptionResponse = ExceptionResponse.builder().datetime(LocalDateTime.now()).errorMessage(usernameException.getUsernameError()).status(HttpStatus.BAD_REQUEST).build();
		return ResponseEntity.badRequest().body(exceptionResponse);
	}
	@ExceptionHandler(UserNotFoundException.class)
	public ResponseEntity<ExceptionResponse> handlerUserNotFoundException(UserNotFoundException userNotFoundException)
	{
		ExceptionResponse exceptionResponse = ExceptionResponse.builder().datetime(LocalDateTime.now()).errorMessage(userNotFoundException.getUserNotFound()).status(HttpStatus.NOT_FOUND).build();
	...
	{
		ExceptionResponse exceptionResponse = ExceptionResponse.builder().datetime(LocalDateTime.now()).errorMessage(loginFailedException.getLoginFailed()).status(HttpStatus.UNAUTHORIZED).build();
	...
	{
		ExceptionResponse exceptionResponse = ExceptionResponse.builder().datetime(LocalDateTime.now()).errorMessage(jwtTokenException.getMessage()).status(HttpStatus.FORBIDDEN).build();
